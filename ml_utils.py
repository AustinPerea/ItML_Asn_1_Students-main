...
class edaDF:
    ...
    def pairPlots(self, splitTarg=False, show=True):
            if splitTarg == False:
                sns.pairplot(data=self.data)
            if splitTarg == True:
                sns.pairplot(data=self.data, hue=self.target)
    
    def boxPlots(self, show=True):
    ...
    def fullEDA(self):
        ...
        with out4:
            fig4 = self.pairPlots(splitTarg=True, show=False)
            plt.show(fig4)

        with out5:
            fig5 = self.boxPlots(show=False)
            plt.show(fig5)
